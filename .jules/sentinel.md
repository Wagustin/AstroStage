...
## 2026-07-14 - Unhandled Exceptions / Denial of Service Risk in API Input
**Vulnerability:** The Vercel API endpoint `api/subscribe.ts` was vulnerable to unhandled TypeErrors (resulting in 500 crashes and potential DoS) because it destructured `email` from `req.body` and called `.includes('@')` on it without verifying that `req.body` existed or that `email` was a string.
**Learning:** Vercel API endpoints receive raw request objects. Always validate the existence and type of `req.body` and its fields before destructuring or invoking methods.
**Prevention:** Validate `req.body` is an object and explicitly verify the types of its properties (e.g., `typeof email === 'string'`) before using them.
