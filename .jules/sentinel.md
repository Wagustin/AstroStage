...
## 2026-08-11 - [Type Validation in Vercel Endpoints]
**Vulnerability:** Server crash due to unhandled TypeError when accessing missing/incorrect types on req.body (e.g. email.includes('@') when email is an array or object).
**Learning:** Vercel functions process raw payloads, leading to potential DoS if unexpected types are submitted and not explicitly checked before applying string methods.
**Prevention:** Always use optional chaining (req.body?.field) and strict type checking (typeof field === 'string') before manipulating input data in API routes.
