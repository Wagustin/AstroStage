...
## 2026-07-31 - Unhandled Exception (Denial of Service) via Missing Input Validation
**Vulnerability:** The API endpoint `api/subscribe.ts` destructured `req.body` and called `email.includes()` without validating if `req.body` existed and was an object, and without checking if `email` was a string. This could lead to a `TypeError` causing a 500 Internal Server Error (server crash).
**Learning:** Vercel serverless functions receive raw request objects. Always validate the existence and type of `req.body` and its fields before destructuring or invoking methods on them to prevent unhandled TypeErrors.
**Prevention:** Implement type validation (e.g., `typeof req.body === 'object'`, `typeof email === 'string'`) before attempting to parse or manipulate request payloads.
