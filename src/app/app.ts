...
export class App implements AfterViewInit {
  ...
  initCursorTrail() {
    ...
    // Cache DOM element and bounding rect position to avoid DOM querying and layout thrashing on every mousemove
    let expSection: HTMLElement | null = null;
    let expSectionTop = 0;

    const updateExpSectionTop = () => {
      if (!expSection || !expSection.isConnected) {
        expSection = document.getElementById('experiencia');
      }
      if (expSection && expSection.isConnected) {
        expSectionTop = expSection.getBoundingClientRect().top;
      } else {
        expSection = null;
        expSectionTop = -1;
      }
    ...
    updateExpSectionTop();
    window.addEventListener('resize', updateExpSectionTop);
    window.addEventListener('scroll', updateExpSectionTop, { passive: true });
    window.addEventListener('popstate', updateExpSectionTop);
    ...
    window.addEventListener('mousemove', (e) => {
      ...
      if (expSection) {
        if (!expSection.isConnected) {
          updateExpSectionTop();
        }
        if (expSection && mouseY < expSectionTop) {
          return;
        }
      }
