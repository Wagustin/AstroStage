...
export class App implements AfterViewInit {
  ...
  initCursorTrail() {
    ...
    // Cache section element and position to avoid layout thrashing on every mousemove event
    let expSection: HTMLElement | null = null;
    let expDocTop: number | null = null;
    ...
    const updateExpTop = () => {
      ...
      if (expSection) {
        expDocTop = expSection.getBoundingClientRect().top + window.scrollY;
      }
    };
    ...
    window.addEventListener('mousemove', (e) => {
      ...
      if (expDocTop !== null) {
        ...
        if (mouseY < expDocTop - window.scrollY) {
          return;
        }
      }
