...
export class App implements AfterViewInit {
  ...
  initCursorTrail() {
    ...
    const updateExpSectionPos = () => {
      if (!expSection) {
        expSection = document.getElementById('experiencia');
      }
      if (expSection) {
        expTopInDoc = expSection.getBoundingClientRect().top + window.scrollY;
      }
    };
