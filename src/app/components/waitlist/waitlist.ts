  ...
  onSubmit() {
    ...
    if (this.waitlistForm.valid) {
      ...
      this.http.post('/api/subscribe', { email }).subscribe({
        ...
        error: (err) => {
          // Log general error for debugging without displaying raw backend details to user
          console.error('Error suscribiendo:', err);
          // Security: Prevent exposure of raw backend/system error details to end users
          this.errorMessage =
